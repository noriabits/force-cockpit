...
- **Browse fields (🗂 Fields)** — for when you don't know what's on the object at all, not just what to type. Opens a full-width drawer below the editor listing every field on the object your query selects from: Type, whether it's Required/Custom/Unique/an External Id, whether it's usable in a `WHERE`/`ORDER BY`/`GROUP BY` clause, and its Help Text (where the admin set one, shown last so a long one never pushes the other columns around) — updated automatically as you change the `FROM` clause or switch tabs. Drag its bottom edge to resize how much of it you see. Tick a field's checkbox to add it to the `SELECT` list; untick to remove it — your query's own formatting and line breaks are left untouched. A lookup field (e.g. `Owner`) expands with **›** to show the fields reachable through it, so ticking one writes the full dotted path (`Owner.Name`). A picklist field expands to show its literal values — click one to insert it as a quoted `WHERE` literal at your cursor. Use the object button at the top of the panel to browse a **different** object's schema without changing your query — while doing that, checkboxes are hidden (a field from another object can't be added to this query) and a banner offers a one-click way back.
