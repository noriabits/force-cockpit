 ...
 * plugins/ and their private/ counterparts) and drops a `.gitignore` (`*`)
 * inside private/ so its contents are never committed. No-ops on a relative path.
...
export function ensureUserFolders(userBasePath: string): void {
  ...
  fs.mkdirSync(path.join(userBasePath, 'plugins'), { recursive: true });
  ...
  fs.mkdirSync(path.join(userBasePath, 'private', 'plugins'), { recursive: true });
