...
export const yamlScriptsFeature: FeatureModuleFactory = (ctx) => {
  const { connectionManager, gateway, workspaceSearch, skillsRepo, postToWebview } = ctx;
  // YamlScriptsService still takes the four script directories as its own
  // bag — it is a service, not a feature, and has no business knowing about
  // the wider context.
  //
  // `ctx.paths` carries the cockpit BASE dirs; the `scripts` sub-path is this
  // feature's to add. Passing a base through raw is silently destructive:
  // `loadYamlItems` walks `{category}/{sub-category}/*.yaml` from whatever it
  // is given, so the base would make `monitoring/`, `logs/` and
  // `.describe-cache/` look like script categories, and `ScriptRepository`
  // would then save into `force-cockpit/{category}/` where the next load
  // cannot find it. Covered by paths.test.ts.
  const paths = {
    builtInPath: path.join(ctx.paths.builtIn, 'scripts'),
    userPath: path.join(ctx.paths.user, 'scripts'),
    privatePath: path.join(ctx.paths.private, 'scripts'),
  ...
  return {
    ...
    routes: {
      ...
      saveYamlScript: {
        handler: async (msg) => {
          const saved = service.saveScript(msg.input as SaveScriptInput, msg.isPrivate as boolean);
          return { script: saved };
        },
      ...
      openScriptResultMarkdown: {
        handler: async (msg) => {
          ...
          const title = ((msg.title as string) || (msg.scriptId as string) || 'AI result').replace(
            /[\\/]/g,
            '_',
          );
          const uri = vscode.Uri.from({
      ...
      loadFavorites: {
        handler: async () => {
          const favorites: string[] = ctx.workspaceState.get('yamlScripts.favorites', []);
          return { favorites };
        },
      ...
      toggleFavorite: {
        handler: async (msg) => {
          ...
          await ctx.workspaceState.update('yamlScripts.favorites', favorites);
          return { favorites };
        },
