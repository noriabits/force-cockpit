// Debug Logs feature factory. The shared AI gateway, describe service,
// workspace search, workspaceState and the logs directory all come from the
// FeatureContext.
...
import type { ApexLogRow, CategoryLevels, TraceLogType } from './types';

export const debugLogsFeature: FeatureModuleFactory = (ctx) => {
  const { connectionManager } = ctx;
  // Initial value only — this is a one-shot read. A live config.yaml reload is
  // picked up by the `setNoiseOptions` call in `loadDebugLogsSetup` below,
  // which is what makes `getConfig` a getter worth having. Do not delete that
  // call as redundant.
  const service = new DebugLogsService(connectionManager, ctx.getConfig().debugLogNoise);
  const stateStore = createDebugLogsStateStore(ctx.workspaceState);
  ...
  return {
    ...
    routes: {
      loadDebugLogsSetup: {
        handler: async () => {
          // THE live-reload point for the noise filter — the constructor above
          // only seeds it once, at activation.
          service.setNoiseOptions(ctx.getConfig().debugLogNoise);
      ...
      saveApexLogAnalysis: {
        handler: async (msg) => {
          ...
          const file = path.join(ctx.paths.logs, `apexlog-${logId}-analysis.md`);
