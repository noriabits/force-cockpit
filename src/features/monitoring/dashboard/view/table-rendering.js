...
/** Characters that fit on one line at `.monitoring-table-td`'s 320px cap (view.css). */
    ...
    for (const row of sorted) {
      ...
      for (const cell of row) {
        ...
        if (isSalesforceRecordId(cell)) {
          ...
          td.textContent = cell;
          // CSS clamps the cell to one ellipsized line (.monitoring-table-td);
          // a click on a long one unwraps it — see scrollEl's click handler.
