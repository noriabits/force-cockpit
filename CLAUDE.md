...
│   │       ├── folder-combobox.js        # createFolderCombobox(...) / buildFolderCombobox(...) — text input + ▾ toggle + dropdown of existing folders (yaml-scripts wires static HTML; monitoring builds nodes dynamically)
│   │       ├── list-filter.ts            # applyListFilter({ elements, getAttrs, matches, query, display? }) — pure per-element visibility loop (folder/source/id + text match) → visibleCount. Unit-tested
│   │       ├── paste-input.js            # wrapWithPasteButton(inputEl, { textarea? }) — builds the `.input-with-paste` wrapper + 📋 button (delegated click handler in paste-buttons.js does the work)
│   │       └── scroll-highlight.js       # scrollAndHighlight(container, selector, highlightClass, durationMs?) — rAF scrollIntoView + timed highlight class toggle
│   ├── utils/
│   │   └── clone-user/
...
│   ├── QueryService.ts           # SOQL query execution (Overview tab); runQuery(soql, useToolingApi?)
│   ├── QueryStateStore.ts        # Quick Query tabs/history/saved queries, persisted in workspaceState (injected Memento; pure, unit-tested)
│   ├── DescribeService.ts        # Cached + projected describeGlobal/describeSObject for SOQL autocomplete (keyed per orgId)
...
- `media/modules/*.js` — Core webview modules, each a self-contained IIFE. Load synchronously in the order declared in `WebviewAssets.WEBVIEW_MODULES` (extension-root-relative paths now — `media/modules/ipc.js`, …, plus the one bundled exception `dist/webview/query-editor.js`; ipc.js first, so every other module can call `win.__onMessage(type, handler)` to subscribe). They must finish before `main.js` runs, which in turn runs before any feature `view.js` (feature scripts are `defer`):
  ...
  - `dist/webview/query-editor.js` — SOQL Quick Query. **Not a `media/modules/` IIFE** — it is an esbuild-bundled ES module whose source lives at `src/webview/query-editor/` (so it can `import` shared TS helpers). Entry `index.js` is an orchestrator that wires the sub-modules below, owns the run/clear/export handlers, the Tooling-API checkbox, and the message handler registrations; posts `loadQueryState` on load; exposes `win.__clearQueryResults` (clears results + the describe cache) for org-lifecycle to call on disconnect. Sub-modules:
    ...
    - `autocomplete/autocomplete.js` — `createAutocomplete(...)`: computes the cursor context, lazily walks relationship paths (`relationshipName → referenceTo[0] → describe`), renders the suggestion dropdown anchored below the textarea, ↑/↓/Enter/Tab/Esc + Ctrl+Space, inserts via `textarea.setRangeText`.
...
**Build pipeline**: `npm run build` runs `scripts/copy-feature-assets.js` first, which copies all non-TS files from `src/features/` → `dist/features/` (skipping any `view/` subfolders — those are ES module sources bundled separately by esbuild). esbuild then runs four times: once for the extension TypeScript (`src/extension.ts` → `dist/extension.js`), once for the yaml-scripts webview (`src/features/utils/yaml-scripts/view/index.js` → `dist/features/utils/yaml-scripts/view.js`, IIFE format), once for the monitoring dashboard webview (`src/features/monitoring/dashboard/view/index.js` → `dist/features/monitoring/dashboard/view.js`, IIFE format), and once for the Overview Quick Query webview (`src/webview/query-editor/index.js` → `dist/webview/query-editor.js`, IIFE format). All webview bundles can `import` from shared TS utilities under `src/utils/` and `src/features/shared/view/` (e.g. `salesforce.ts`, `table-sort.ts`) — esbuild resolves `.ts` extensions automatically. The VSIX packages `dist/features/` and `dist/webview/` alongside `dist/extension.js`.
...
**`MessageRouter.ts`** — Dispatches incoming messages. Built-in host routes (`ready`, `query`, `openRecord`, `openExternalUrl`, `exportQueryResult`, `loadQueryState`, `saveQueryTabs`, `addQueryHistory`, `saveSavedQueries`, `describeGlobal`, `describeSObject`, `openInBrowser`, `confirmAction`, `operationStarted/Ended`, `cancelOperation`) are handled inline. `exportQueryResult` writes the Quick Query export (`{ content, format }`) to a timestamped `query-result-{YYYYMMDD-HHmmss}.{csv|json}` in the workspace root, then opens it in the editor (errors — no workspace folder / write failure — surface via `showErrorMessage`). The `query` route accepts an optional `useToolingApi` flag (routed to `QueryService.runQuery`). The query-state routes delegate to `QueryStateStore` (workspaceState-backed tabs/history/saved queries); the `describe*` routes delegate to `DescribeService` (cached, projected describe metadata for autocomplete). `MainPanel` constructs `QueryStateStore(context.workspaceState)` + `DescribeService(connectionManager)` and injects both. Any other type falls through to feature routes registered via `defineFeature()`. The private `_route(action, successType, errorType, context)` helper echoes `opId` (and anything else in `context`) into both success and error responses so the webview can correlate.
...
**Other shared webview primitives** (`src/features/shared/view/`, all import-only, bundled per-feature by esbuild): besides the filter bar, both webviews share four small modules — (1) `folder-combobox.js`: `createFolderCombobox({ wrapper, input, toggleBtn, dropdownEl, optionClass, getFolders, onSelect? })` wires pre-existing DOM (yaml-scripts' static combobox in `view.html`); `buildFolderCombobox({ classPrefix, value?, placeholder?, inputId?, getFolders, onSelect? })` first creates the wrapper/input/toggle/dropdown then delegates (monitoring's dynamic edit form). Both return `{ refresh, open, close, cleanup }` (`cleanup()` removes the document click-outside listener — monitoring calls it on edit-form teardown via the `cleanups` list; yaml-scripts ignores it). Free-text is always allowed; CSS stays per-feature via `classPrefix`/`optionClass`. (2) `list-filter.ts`: pure `applyListFilter(...)` (see tree) — each consumer keeps its own no-results toggle + feature-specific concerns (yaml-scripts passes `id` for favorites; monitoring hides drag handles when filtered) at the call site. (3) `paste-input.js`: `wrapWithPasteButton(...)`. (4) `scroll-highlight.js`: `scrollAndHighlight(...)` — used by yaml-scripts' post-save flow (monitoring may adopt it later with a `.monitoring-card--highlight` animation).
...
**JS scripts** execute in a Node.js `vm` sandbox (`createContext` + `Script`). The context provides: `connection` (raw jsforce `Connection` or `null`), `org` (`OrgDetails` or `null`), `query(soql)` (convenience wrapper), `executeApex(apexBody, options?)` (runs anonymous Apex via the SOAP path — `ConnectionManager.executeAnonymousWithDebugLog` — so it puts the Apex in the POST body and avoids the HTTP 431 that `connection.tooling.executeAnonymous` hits on large scripts; returns `{ compiled, success, compileProblem, exceptionMessage, exceptionStackTrace, debugLog }`), `log(...args)` / `error(...args)` (captured as output), `console` (`{ log, error, warn }`), `fs`, `path`, `yaml`, `setTimeout`, `clearTimeout`, `Promise`. The code is wrapped in an async IIFE so `await` works at top level. Output from `log()` / `console.log()` is returned as the debug log. JS scripts show a green `[JS]` badge, their Execute button is always enabled (no org needed), and no USER_DEBUG checkbox is shown.
...
- `execution/JsExecutor.ts` — runs user JS in a Node.js `vm` sandbox (`createContext` + `Script`). Context: `connection`, `org`, `query`, `executeApex`, `log`/`error`, `console`, `fs`, `os`, `path`, `yaml`, `xmlFormat`, `DOMParser`, `XMLSerializer`, `xml`, `input`, `xmlEscape`, `setTimeout`, `clearTimeout`, `Promise`. `executeApex(apexBody, options?)` delegates to `ConnectionManager.executeAnonymousWithDebugLog` (SOAP — Apex in the POST body, avoiding the HTTP 431 that `connection.tooling.executeAnonymous` hits on large scripts) and returns the execution result plus `debugLog`. Async IIFE wrap so top-level `await` works. `AbortSignal` via `Promise.race`.
...
- `view/` — webview source as ES modules. **Bundled** by a dedicated esbuild call into `dist/features/utils/yaml-scripts/view.js` (single `<script>` loaded by `WebviewAssets`). `scripts/copy-feature-assets.js` skips `view/` directories so the raw sources never reach `dist/`. Sub-modules:
  - `view/index.js` — orchestrator (~420 lines): DOM refs for the list/filter area, top-level state (`connected`, `currentOrgData`, `favoriteIds`, `currentScripts`, the op-tracking Maps `opIdToScriptId`/`scriptLogContent`/`executeStateUpdaters` — single owner), the render loop (`renderScripts`), execute-result/log-chunk handlers, message handler map, feature registration. Wires the sub-modules below via factory invocations. Category/visibility filtering is delegated to the shared `createCategoryFilterBar` from `src/features/shared/view/category-filter-bar.js`.
  - `view/script-form.js` — `createScriptForm({ labels, vscode, hljs, filterBar, getCurrentScripts })` returns `{ showNewForm, showEditForm, hideNewForm, refreshFolders, onSaveError, onDeleteError, setFilePath }`. Owns the entire new/edit form: form DOM refs + label init, the code-editor and form-inputs-editor instances, the shared folder combobox, `editingScriptId`/`editingScriptSource` state, validation, and `saveYamlScript`/`updateYamlScript`/`deleteYamlScript` dispatch. The form-result messages stay dispatched from `index.js`'s handler map and delegate to the returned hooks.
  ...
  - `view/log-viewer.js` — `createLogViewer({ labels, vscode, renderLogWithLinks, renderLogWithJsonTables })` returns `{ buildLogViewer(script) }` → `{ fragment, refs }`. Builds the per-script log viewer: status hint, error box, log output (with external-link click handling), the apex-only USER_DEBUG / Format JSON filter bar honoring the script's YAML defaults, and the Open-in-editor + Copy buttons. Exports the `LogViewerRefs` typedef used by the other modules.
  - `view/execute-handler.js` — `createExecuteHandler({ labels, vscode, opIdToScriptId, getConnected, getCurrentOrgData, startAction, confirmIfSensitive })` returns `{ attachExecuteHandler({ script, section, executeBtn, needsOrg, inputFields, refs }) }`. Wires the Execute button: input collection, `__confirmIfSensitive` BEFORE `__startAction`, opId tracking and `executeYamlScript` dispatch.
  - `view/accordion-builder.js` — `createAccordionBuilder(ctx)` returns `{ buildAccordion, updateFavoriteStars }`. Builds per-script accordion structure: header (favorite star, trigger, type/private/invalid badges, edit button), execution input fields (`buildInputFields`, using the shared `wrapWithPasteButton`), invalid-script rendering, and body composition. The log viewer and execute handler are injected via ctx (`buildLogViewer`, `attachExecuteHandler` — wired in `index.js` from the two factories above) alongside `labels`, `scriptsList`, `favoriteIds`, `executeStateUpdaters`, `getConnected`, `onEditClick`, `vscode`, `escapeHtml`, so the module never reaches into `view/index.js`'s scope directly.
...
- [BackgroundRefresher.ts](src/features/monitoring/dashboard/BackgroundRefresher.ts) — `BackgroundRefresher` class with `start(configs)`, `stop()`, `restart(configs)`. Filters configs to only those that match `hasNotifications(cfg)` (imported + re-exported from the shared leaf [notification-config.ts](src/features/monitoring/dashboard/notification-config.ts)) and have `refreshInterval > 0`. Skips `__preview__*` ids. Enforces the same 10s minimum as the webview. Each tick checks `connectionManager.isConnected`, calls `service.runQuery` (chart/metric) or `service.runTableQuery` (table), runs `checkThresholds` + `fireBreachNotifications` + `checkRowCountIncrease` + `fireRowCountNotifications` from `notifications.ts`, and posts `monitoringBackgroundRefreshResult` to the panel (no-op when the panel is closed).
- [notifications.ts](src/features/monitoring/dashboard/notifications.ts) — single shared module owning `notificationCooldowns` and `previousRowCounts` Maps. Imported by both the route handlers and the BackgroundRefresher so cooldowns and row-count baselines stay consistent across host/route/refresher.
...
- The webview's `refresh-scheduler` (driven from [view/index.js](src/features/monitoring/dashboard/view/index.js)) skips its own `setupAutoRefresh` timer for any config where `hasNotifications(cfg)` is true (same shared `notification-config.ts` predicate as the host) — the host owns those. It also handles a new `monitoringBackgroundRefreshResult` message that routes through the existing `onQueryResult` / `onTableQueryResult` render path. The `panelVisibilityChanged` resume hook also skips notification configs to avoid double-firing on visibility regain.
...
- **Audio cue**: row-count growth notifications also play a short OS sound. The cue lives in [audio.ts](src/features/monitoring/dashboard/audio.ts) (`playRowCountPing`) and is invoked by `fireRowCountNotifications` in `notifications.ts` so every code path (route handler + BackgroundRefresher) gets it for free. It uses `child_process.spawn` per `process.platform`: `afplay` (macOS, Glass.aiff), `powershell.exe -Command [console]::beep(880,300)` (Windows), `paplay` (Linux, freedesktop message.oga). All failures (binary missing, audio device unavailable) are swallowed — best-effort. The webview's old Web Audio `playNotificationPing` was removed; the host now owns the cue so it plays even when the panel is closed.

**`view/` sub-modules** (mirrors the yaml-scripts split): the webview source is ES modules bundled by the existing esbuild call (entry stays `view/index.js`). Each DOM module is a factory receiving a `ctx` so it never reaches into `index.js` scope; pure helpers are DOM-free TypeScript so they can be unit-tested.
- `view/index.js` — orchestrator (~460 lines): state (`connected`, `isVisible`, `configs`, `connectedOrgId`, `pendingInitialLoad`, `searchQuery`, the `chartInstances` Map — single owner), DOM refs, the shared `createCategoryFilterBar`, all factory wiring, the IntersectionObserver visibility hook, feature registration + message handler map (delegating to the module methods below), `renderAll` + `isFiltered`/`applyFilters`, `switchToEditMode` + `addNewCard`, the save handlers (`onSaveResult`/`onSaveError`/`findEditingCard` — bridge the `__pendingSave*` edit-form contract to `configLoader.loadConfigs`), and the grid-scoped UI helpers (`setCardStatus`/`setCardError`/`setEditStatus`/`findCardTypeSelect`/`findEditCard`/`setAllButtonsDisabled`) passed as ctx callbacks to the renderers/query-runner. Wires the sub-modules below via factory invocations.
- `view/chart-rendering.js` — `createChartRenderer({ chartInstances, labels, setCardStatus })` returns `{ renderChart, switchChartType }`. Also exports the `CHART_COLORS`, `CHART_TYPES_WITH_CANVAS`, and `ALL_CHART_TYPES` constants. Owns the Chart.js create/destroy lifecycle but **not** the `chartInstances` Map — that is injected via ctx so there is a single owner (see Risks).
- `view/table-rendering.js` — `createTableRenderer({ grid, labels, setCardStatus, vscode })` returns `{ renderTable, renderTableInEl }`. Owns the table build, the `wrapper._tableState` stash, sort headers, per-table filter + match counter, and record-id link cells. Imports `isSalesforceRecordId` from `src/utils/salesforce.ts` and the pure `filterRows`/`sortRows` from `src/features/shared/view/table-sort.ts` (shared with the Overview Quick Query results table).
- `view/edit-form.js` — `createEditForm(ctx)` returns `{ buildEditForm }`. Largest module (~600 lines): folder combobox, value-field rows, scalar/value-field readers, the 800ms preview debounce (owns its own `debounceTimers` Map — no longer injected), and save/cancel/delete wiring. Sets the `card.__pendingSaveResolve` / `__pendingSaveError` / `__pendingSaveResolveCleanups` hooks that the message handlers in `index.js` invoke — that contract is preserved.
- `view/drag-order.js` — `createDragOrder({ grid, getConfigs, vscode })` returns `{ init, makeCardDraggable, nextAvailablePosition, saveCardOrder }`. Owns the `dragSrcId` so the grid-level live-swap dragover (`init`) and the per-card dragstart/dragend (`makeCardDraggable`) stay in one place.
- `view/query-runner.js` — `createQueryRunner(ctx)` returns `{ triggerQuery, onQueryResult, onQueryError, onTableQueryResult }`. Owns query dispatch + all result/error routing + metric rendering (preview vs. live, chart/metric/table). `triggerQuery(cfg)` takes the full config (every call site holds it). The host's `monitoringBackgroundRefreshResult` path is routed through the returned `onQueryResult`/`onTableQueryResult` from the index.js handler map.
- `view/refresh-scheduler.js` — `createRefreshScheduler({ getConnected, getIsVisible, triggerQuery })` returns `{ setupAutoRefresh, clearAllRefreshTimers }`. **Owns the `refreshTimers` Map.** Skips notification-enabled configs (the host owns those — see `hasNotifications` below); enforces the 10s minimum interval; each tick is gated on live connection + panel visibility.
- `view/card-builder.js` — `createCardBuilder(ctx)` returns `{ buildViewCard }`. Builds the view-mode card shell (outer card + data-* attrs, header with drag handle / title / private badge / type selector / Edit + Refresh buttons, content area, status + error box). The Edit button calls the injected `onEditClick` (index.js's `switchToEditMode`); the type selector calls `chartRenderer.switchChartType`. Imports `CHART_TYPES_WITH_CANVAS` from `chart-rendering.js`.
- `view/config-loader.js` — `createConfigLoader(ctx)` returns `{ loadConfigs, onConfigsLoaded, onDeleteResult, onDeleteError, showLoadError }`. Owns config load/sort (position, id tiebreak) handed back to the orchestrator via the `applyConfigs` callback (so `configs` keeps a single owner in index.js), the "Restore hidden built-ins" link, the load-error box, and delete result/error handling (`onDeleteError` still invokes the edit-form's `__pendingSaveError`).
- `view/format-value.ts` — pure `formatValue(value, format)` (currency / percent / locale number), shared by chart axes/tooltips, metric tiles, and tables. Unit-tested in `format-value.test.ts`.
- `view/metric-value.ts` — pure `extractMetric(data, valueFields)` → `{ empty } | { empty, text, label }`: the metric-card empty-dataset check + formatted value/label selection (DOM-free; the DOM write stays in query-runner's `renderMetricInEl`). Unit-tested in `metric-value.test.ts`.
- `src/features/shared/view/table-sort.ts` — pure `filterRows(rows, query)` (case-insensitive substring) + `sortRows(rows, sortCol, sortAsc)` (numeric-aware, locale fallback). Shared by the monitoring table renderer and the Overview Quick Query results table. Unit-tested in `table-sort.test.ts`.
- `notification-config.ts` (dashboard root, not `view/`) — pure `hasNotifications(cfg)` leaf module (no `vscode` import) shared by the host `BackgroundRefresher` (which re-exports it) and the webview `refresh-scheduler`/`index.js`, so both agree on who owns the timer. Unit-tested in `notification-config.test.ts`.
...
- **Paste-from-clipboard button** — wrap any `<input type="text/email">` in a `<div class="input-with-paste">` and add `<button type="button" class="paste-btn" title="Paste from clipboard">📋</button>` immediately after the input. A single delegated handler in `media/main.js` handles all paste buttons. For inputs inside flex-row containers (`.clone-search-row`, `.ro-filter-row`), add `style="flex: 1; min-width: 0"` to the wrapper to preserve the horizontal layout. Selects and checkboxes do not get paste buttons. `<textarea>` inputs in static HTML do not get paste buttons, but `textarea` execution inputs in yaml-scripts do — use `<div class="input-with-paste input-with-paste--textarea">` (block + relative positioning) with the paste button positioned absolute top-right. For dynamically generated inputs (yaml-scripts execution inputs), build the wrapper via the shared `wrapWithPasteButton(inputEl, { textarea? })` helper from `src/features/shared/view/paste-input.js` instead of hand-rolling the wrapper + button (the delegated handler in `paste-buttons.js` covers the click).
...
- **Unit tests** use **Vitest** (`npm test`). Test files live alongside the source as `*.test.ts`. All tests are pure unit tests — no real Salesforce org, no network calls. Mock pattern: `function makeMock(overrides = {}): ConnectionManager { return { ... } as unknown as ConnectionManager; }`. Private methods are accessed via `(service as any).method()`. Files with `beforeEach`/`afterEach` must place them inside a `describe` block (not at module scope) to avoid a Vitest v4 runner crash. Test files with test coverage: `apexUtils.ts`, `CloneUserService.ts`, `ReactivateOmniscriptService.ts`, `YamlScriptsService.ts`, `MonitoringDashboardService.ts`, `dashboard/parsing/MonitoringConfigParser.ts`, `dashboard/persistence/MonitoringConfigRepository.ts`, `utils/yamlRepository.ts`, `config.ts`, `category-filter-state.ts`, `shared/view/list-filter.ts`, `OrgConnectionController.ts`, `orgType.ts`, `workspaceSetup.ts`, `dashboard/view/format-value.ts`, `shared/view/table-sort.ts`, `dashboard/view/metric-value.ts`, `dashboard/notification-config.ts`, `BackgroundRefresher.ts`, `notifications.ts`, `salesforce/soap/SoapEnvelope.ts`, `salesforce/soap/SoapResponseParser.ts`, `panels/OperationRegistry.ts`, `panels/MessageRouter.ts`, `webview/query-editor/export-format.ts`, `webview/query-editor/autocomplete/soql-context.ts`, `services/QueryService.ts`, `services/QueryStateStore.ts`, `services/DescribeService.ts`, `salesforce/connection.ts`.
