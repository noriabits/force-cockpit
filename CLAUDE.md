...
- `view/index.js` — orchestrator (~700 lines): state, DOM refs, the shared `createCategoryFilterBar`, card shells (`buildViewCard`/`buildCardHeader`/`buildTypeSelect`), query dispatch (`triggerQuery`), preview-result routing, metric rendering, auto-refresh timers, and feature registration / message dispatch. Wires the four sub-modules below via factory invocations.
