...
- `view/table-sort.ts` — pure `filterRows(rows, query)` (case-insensitive substring) + `sortRows(rows, sortCol, sortAsc)` (numeric-aware, locale fallback). Unit-tested in `table-sort.test.ts`.
